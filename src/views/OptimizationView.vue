<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        <h2 class="text-2xl font-bold text-white">{{ $t('optimization.title') }}</h2>
        <p class="text-steel-400 mt-1">{{ $t('optimization.description') }}</p>
      </div>
      <button @click="handleOptimize" class="btn-primary" :disabled="optimizing">
        <Loader2 v-if="optimizing" :size="18" class="animate-spin" />
        <Play v-else :size="18" />
        {{ optimizing ? $t('optimization.calculating') : $t('optimization.run') }}
      </button>
    </div>
    ...
    <div v-if="optimizing" class="card p-12 flex flex-col items-center justify-center">
      ...
      <h3 class="text-lg font-semibold text-white mb-1">{{ $t('optimization.optimizing') }}</h3>
      <p class="text-steel-400 text-sm">{{ $t('optimization.algorithm') }}</p>
    </div>
    ...
    <div v-else-if="!result" class="card p-12 flex flex-col items-center justify-center">
      ...
      <h3 class="text-lg font-semibold text-steel-300 mb-1">{{ $t('optimization.ready') }}</h3>
      <p class="text-steel-500 text-sm text-center max-w-md">
        {{ $t('optimization.readyDescription') }}
      </p>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="card p-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-sm text-steel-400 font-medium">{{ $t('optimization.projectedProfit') }}</p>
            <p class="text-3xl font-bold text-forge-400">
              {{ formatCurrency(result.totalProjectedValue) }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="card overflow-hidden">
        <div class="px-6 py-4 border-b border-steel-800">
          <h3 class="text-lg font-semibold text-white">{{ $t('optimization.suggestions') }}</h3>
          <p class="text-steel-400 text-sm mt-0.5">{{ $t('optimization.suggestionsSub') }}</p>
        </div>

        <div v-if="result.productionSuggestions.length === 0" class="p-8 text-center">
          <p class="text-steel-500 text-sm">{{ $t('optimization.noSuggestions') }}</p>
        </div>

        <table v-else class="w-full">
          <thead>
            <tr class="border-b border-steel-800">
              <th class="table-header text-left py-4 px-6">{{ $t('optimization.product') || $t('products.product') }}</th>
              <th class="table-header text-left py-4 px-6">{{ $t('optimization.suggestedQty') }}</th>
              <th class="table-header text-left py-4 px-6">{{ $t('optimization.unitPrice') }}</th>
              <th class="table-header text-right py-4 px-6">{{ $t('optimization.subtotal') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              ...
              <td class="table-cell text-steel-300">
                {{ formatCurrency(suggestion.product.salePrice) }}
              </td>
              <td class="table-cell text-right font-semibold text-white">
                {{ formatCurrency(suggestion.quantity * suggestion.product.salePrice) }}
              </td>
            </tr>
          </tbody>
        </table>
